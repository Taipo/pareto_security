...
class pareto_settings extends pareto_functions {
	...
	function __construct() {
		...
		$unix_time = $this->updated( 1526089273, ( int )get_option( 'gmt_offset' ) );
		$this->time_zone = date_default_timezone_get() . get_option( 'gmt_offset' );
		
		define( 'PARETO_VERSION', '1.8.9' );
		...
		if ( empty( $this->logs ) ) {
			$this->logs = array( 0 => str_replace( ' ', '%20', PARETO_RELEASE_DATE ) . " Safe 127.0.0.1 GET plugins.php Pareto%20Security%20Installed" );
		}
		...
		if ( isset( $this->options[ 'safe_list' ] ) ) {
			$domain_list = $this->host_check( $this->options[ 'safe_list' ] );
			if ( false === strpos( $domain_list, $this->get_http_host() ) ) $domain_list = $this->get_http_host() . "\n" . $domain_list;
			$this->options[ 'safe_list' ] = $domain_list;
		}
		...
		if ( false !== ( bool )$this->is_wp( true ) ) {
			if ( $_SERVER[ 'REQUEST_METHOD' ] == 'POST' ) {
				...
				if ( isset( $_POST[ $this->settings_field ][ "ban_mode" ] ) ) $_POST[ $this->settings_field ][ "ban_mode" ] = ( int ) $_POST[ $this->settings_field ][ "ban_mode" ];
				if ( isset( $_POST[ $this->settings_field ][ "advanced_mode" ] ) ) $_POST[ $this->settings_field ][ "advanced_mode" ] = ( int ) $_POST[ $this->settings_field ][ "advanced_mode" ];
				if ( isset( $_POST[ $this->settings_field ][ "email_report" ] ) ) $_POST[ $this->settings_field ][ "email_report" ] = ( int ) $_POST[ $this->settings_field ][ "email_report" ];
				if ( isset( $_POST[ $this->settings_field ][ "hard_ban_mode" ] ) ) $_POST[ $this->settings_field ][ "hard_ban_mode" ] = ( int ) $_POST[ $this->settings_field ][ "hard_ban_mode" ];	
			}
		...
		$this->_domain_list = ( array_key_exists( 'safe_list', $this->options ) ) ? $this->get_field_value( $this->options, 'safe_list' ) : null;
	}
	...
	*/
	function render() {
		...
		<div class="wrap">
			...
			<form method="post" action="options.php" <?php if ( false !== ( bool ) $this->_adv_mode && false === ( bool ) $this->_hard_ban_mode ) { ?>onsubmit="return confirm( 'Warning: If you are changing from advanced mode to standard mode log files marked as Low Severity will not be displayed. Click Ok to continue' );"<?php } ?>>
				...
				<tr>
					...
					<ol>
						...
						<li>One domain name per line: (i.e <?php echo $this->get_http_host(); ?> - without <code><?php echo $http; ?></code> scheme/protocol and double forward slashes)</li>
							<?php
								if ( isset( $this->_domain_list  ) ) {
									if ( false === preg_match( "/$this->get_http_host()/i", $this->_domain_list ) ) {
										$this->_domain_list = $this->get_http_host()  . "\n" . $this->_domain_list;
									}
								} else $this->_domain_list = $this->get_http_host() . "\n";
							?>
							<textarea <?php echo ( false === ( bool ) $this->_adv_mode ) ? 'disabled' : ''; ?> name="<?php echo $this->get_field_name( 'safe_list' ); ?>" id="<?php echo $this->get_field_name( 'safe_list' ); ?>" rows="<?php echo ( false === is_null( $this->_domain_list ) ) ? 3 + count( $this->_domain_list ) : 2; ?>" cols="30"><?php echo $this->_domain_list; ?></textarea>
...
?>
			<table style="text-align: left;">
				...
				<tr>
					<td><input type="submit" class="button button-primary" name="save_options" value="<?php esc_attr_e( 'Save Options' ); ?>" /><br /><br /></td>
				</tr>
			</table>
<?php
	}
...
?>
			<table style="text-align: left;">
				<tr>
					<td><strong><?php _e( 'Version:', 'pareto_security_settings' ); ?></strong> <?php echo PARETO_VERSION; ?> <?php echo '&middot;'; ?> <strong><?php _e( 'Released:', 'pareto_security_settings' ); ?></strong> <?php echo PARETO_RELEASE_DATE; ?> ( <?php echo $this->time_zone; ?> )</td>
					...
					<tr style="text-align: left; background-color: #E8E8E8;">
						...
						<td style="width: 500px; vertical-align:top;">
							<table>
								<tr>
								  <td>
								      <input type="hidden" name="<?php echo $this->get_field_name( 'ban_mode' ); ?>" id="<?php echo $this->get_field_id( 'ban_mode' ); ?>" value="1" />
								  ...
								  <td>
								<label for="<?php echo $this->get_field_id( 'advanced_mode' ); ?>"><?php _e( '<b>Set Advanced Mode</b>', 'pareto_security_settings' ); ?></label></td>
								</tr>
